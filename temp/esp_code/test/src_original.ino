...
QueueHandle_t _recv_q;
void receiver_task(void *params) {
  char buf[MAX_BUFFER_LEN];
  while (true) {
    if (Serial.available()) {
      int len = Serial.readBytesUntil('\n', buf, MAX_BUFFER_LEN - 1);
      if (len > 0) {
        buf[len] = '\0';
        Serial.print("Received: ");
        Serial.println(buf);  // Echo what it received
      }      
      
      if (len > 0) {
        buf[len] = '\0'; // Null-terminate
        if (!xQueueSend(_recv_q, (void*)buf, 0)) {
          DEBUG_E("Receive Queue full!");
        }
      }
    }
    vTaskDelay(pdMS_TO_TICKS(1)); // Yield a little
  }
}
...
void send_message(const char *msg) {
  Serial.println(msg); // Automatically appends newline
}
 ...
 void setup(){
  Serial.begin(115200);
  _recv_q = xQueueCreate(10, MAX_BUFFER_LEN);  // Create queue if not already
  if (_recv_q == NULL) {
    DEBUG_E("Failed to create queue!");
  }

  xTaskCreatePinnedToCore(
    receiver_task,     // Function to call
    "receiver_task",   // Task name
    4096,              // Stack size
 ...
 void loop(){
  ...
  //     connect_client(); }
   if (ini == true) //MUDAR ISTO
   {
     memset(incoming_msg, 0, MAX_BUFFER_LEN);
     received = get_message(incoming_msg);
     DEBUG_I("Received: %s", incoming_msg);
     DEBUG_I("Received: %d", received);
   }
   ...
   if(received){
     ...
     else if ((flag[0] == 'F') || (flag[0]=='B') || (flag[0] == 'R') || (flag[0] == 'L') || (flag[0] == 'T') || (flag[0] == 'K') || (flag[0] == 'U')) //4 packet message
     { 
       DEBUG_I("Flag: %s", flag);
 
       send_message(flag);
       strcpy(incoming_msgs_list[0], incoming_msg);
       int n_msgs = 1;
       while (n_msgs <= 3){
         memset(incoming_msg, 0, MAX_BUFFER_LEN);
         received = get_message(incoming_msg);
         if(received and ((incoming_msg[3] - '0') == n_msgs))
         {
           send_message(flag);
           DEBUG_I("Received: %s", incoming_msg);
           strcpy(incoming_msgs_list[n_msgs], incoming_msg);
           n_msgs++;
         }
       }
       DEBUG_I("%s", flag);
       ...
       if (flag[0] == 'F' )
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_F,elbow_list_F,wrist_list_F);
       else if (flag[0] == 'B')
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_B,elbow_list_B,wrist_list_B);
       else if (flag[0] == 'R')
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_R,elbow_list_R,wrist_list_R);
       else if (flag[0] == 'L')
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_L,elbow_list_L,wrist_list_L);
       else if (flag[0] == 'T')
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_RR,elbow_list_RR,wrist_list_RR);
       else if (flag[0] == 'K')
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_RL,elbow_list_RL,wrist_list_RL);
       else if (flag[0] == 'U')
         custom_gait_ini = true;
         set_gait(n_msgs,incoming_msgs_list,shoulder_list_custom,elbow_list_custom,wrist_list_custom);
       int rows, cols; 
       send_message(flag);
       memset(flag,0,MAX_BUFFER_LEN); //talvez alterar flag para ser so char com tamanho 1
 
       rows = sizeof(shoulder_list_F) / sizeof(shoulder_list_F[0]);
       cols = sizeof(shoulder_list_F[0]) / sizeof(double);
 
       DEBUG("double shoulder_list_F[4][nCyclePoints] = {");
       for (int i = 0; i < rows; i++) {
           DEBUG("{");
           for (int j = 0; j < cols; j++) {
               DEBUG("%d", (int) shoulder_list_F[i][j]);
               if (j < cols - 1) {
                   DEBUG(",");
               }
           }
       ...
       // Print elbow_list_F
       rows = sizeof(elbow_list_F) / sizeof(elbow_list_F[0]);
       cols = sizeof(elbow_list_F[0]) / sizeof(double);
 
       DEBUG("double elbow_list_F[4][nCyclePoints] = {");
       for (int i = 0; i < rows; i++) {
           DEBUG("{");
           for (int j = 0; j < cols; j++) {
               DEBUG("%d",(int) elbow_list_F[i][j]);
               if (j < cols - 1) {
                   DEBUG(",");
               }
           }
       ...
       // Print wrist_list_F
       rows = sizeof(wrist_list_F) / sizeof(wrist_list_F[0]);
       cols = sizeof(wrist_list_F[0]) / sizeof(double);
 
       DEBUG("double wrist_list_F[4][nCyclePoints] = {");
       for (int i = 0; i < rows; i++) {
           DEBUG("{");
           for (int j = 0; j < cols; j++) {
               DEBUG("%d",(int) wrist_list_F[i][j]);
               if (j < cols - 1) {
                   DEBUG(",");
               }
           }
