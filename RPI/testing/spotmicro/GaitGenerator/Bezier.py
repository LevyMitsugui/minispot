...
class BezierGait():
    ...
    def SwingStep(self, phase, L, LateralFraction, YawRate, clearance_height,
        ...
        #    coord = np.array([X_delta_lin + X_delta_rot, Y_delta_lin + Y_delta_rot, (Z_delta_lin + Z_delta_rot)*2.5])
        #    print("DEPOIS {},{},{}".format(coord[0],coord[1],coord[2]))
        ...
        for i, (key, Tbf_in) in enumerate(T_bf_.items()):
            ...
            if Tstance > 0.0:
                step_coord = self.GetFootStep(L, LateralFraction, YawRate,
                                              ...
                                              i, key)
                if key == "FL":
                    print("{},{},{}".format(step_coord[0],step_coord[1],step_coord[2]))
            else:
